	...
	try {
		...
		if (supabaseClient) {
			...
			try {
				...
				console.warn('Could not query push_subscriptions table:', e.message);
			}

			// 2. Fallback: fetch from site_settings config if table missing or empty
			try {
				const { data } = await supabaseClient.from('site_settings').select('*').eq('id', 'default').single();
				if (data && data.config && data.config.pushSubscriptions) {
					const fallbackSubs = data.config.pushSubscriptions;
					fallbackSubs.forEach(fs => {
				...
				console.warn('Could not query site_settings fallback:', e.message);
			}
		}
